...
export class Migrations1745376036522 implements MigrationInterface {
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        ...
        // Check if identity_role_enum already exists before creating
        const identityRoleEnumExists = await queryRunner.query(`
            SELECT 1 FROM pg_type 
            WHERE typname = 'identity_role_enum' 
            AND typnamespace = (SELECT oid FROM pg_namespace WHERE nspname = 'public')
        `);
        
        if (identityRoleEnumExists.length === 0) {
        ...
        // Check if enum type already exists before creating

...
export class MinioService {
  ...
  constructor(private readonly configService: ConfigService) {
    ...
    // Construct public URL for accessing files
    const protocol = this.useSSL ? 'https' : 'http';
    this.publicUrl = `${protocol}://${this.endpoint}:${this.port}`;

    this.logger.log(`MinIO client initialized: ${this.publicUrl}`);
  }
  ...
  async getPresignedUrl(
    ...
    try {
      const url = await this.minioClient.presignedGetObject(bucket, objectPath, expirySeconds);
      return url;
    } catch (error) {
